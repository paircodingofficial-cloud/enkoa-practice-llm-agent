...
실행: 이 파일이 있는 폴더에서  uv run 06_종합_데이터분석_자동화3.py
...
import sys
import platform
from contextlib import AsyncExitStack   # 여러 세션을 한 블록에서 겹쳐 열고 한꺼번에 닫는다
...
# 서버 이름과 설정을 한 곳에 모아 둔다. 이 이름이 그대로 도구 이름의 접두사가 된다.
SERVERS = {"db": SQLITE, "code": CODE_RUNNER, "files": FILESYSTEM}
...
async def main():
    ...
    # 세션을 열어 두는 방식이다. 서버 프로세스가 블록이 끝날 때까지 살아 있어,
    # 도구를 부를 때마다 서버를 새로 띄우지 않는다(도구 호출마다 npx·uvx 가 다시 뜨지 않는다).
    # AsyncExitStack 은 async with 를 세 겹 겹쳐 쓰는 대신 반복문으로 쌓아 두고,
    # 블록이 끝나면 연 순서의 역순으로 알아서 닫아 준다.
    async with AsyncExitStack() as stack:
        ...
        for server_name in SERVERS:
            ...
            for tool_item in await load_mcp_tools(session):
                ...
                tool_item.name = f"{server_name}_{tool_item.name}"
