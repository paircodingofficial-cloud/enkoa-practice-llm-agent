...
한곳에 모았습니다. 교안_02 에서 한 줄씩 뜯어본 설정 그대로이고, 이번 과제의 주제는 설정이 아니라
계획·파이프라인·정형화입니다.
"""
...
DAY_DIR = BASE_DIR.parent.parent                    # 일차 폴더(day21). data 와 utils.py 가 있다
...
# 에이전트에 공통으로 넣는 지시. 교안_02 의 SYSTEM_BASE 에 도구 인자 규칙 한 문장을 더한 것이다.
...
PLAN_RULE = (" 요청을 받으면 무엇을 하든 첫 도구 호출은 반드시 write_todos 여야 한다. "
             "할 일을 단계로 나눠 계획을 세운 뒤 하나씩 처리하고, 단계를 끝낼 때마다 상태를 갱신하라.")

# 미들웨어가 끼어들 수 있는 여섯 자리. 교안_02 앞에서 본 순서 그대로다.
